...
public class Methods {
    ...
    //error de servidor no deberia crear sobre otro objeto existente
    public void postNewPlayer(TextArea textArea, TextField textFieldId, TextField textFieldMail, TextField textFieldName) {
        ...
        try {
            ...
            player.accumulate("codJugador", id);
    ...
    //error de servidor ??? not-null property references a null or transient value : entity.Juegos.companiesByDesarrolladora
    public void postNewGame(TextArea textArea, TextField textFieldId_pegi, TextField textFieldMail, TextField textFieldName){

        try {
            HttpPost httpPost = new HttpPost("http://localhost:8080/juegos");
            JSONObject game = new JSONObject();
            int id = Integer.valueOf(textFieldId_pegi.getText());
            String name = textFieldName.getText();
            ...
            game.accumulate("cod_juego",id);
            game.accumulate("nombre", name);
            game.accumulate("fechaLanzamiento", " ");
            game.accumulate("precio", 123);
            game.accumulate("genero", genero);
            game.accumulate("pegi", 15);
            game.accumulate("companiesByDesarrolladora", 233);
            game.accumulate("companiesByDistribuidora", 242);
    ...
    public void deleteGameById (TextArea textArea, TextField idTextField){
        ...
        try {
            ...
            try (CloseableHttpResponse response1 = httpClient.execute(delRequest)) {
                ...
                textArea.setText(response1.getCode() + " " );
    ...
    public void updateGameById (TextArea textArea, TextField textFieldId, TextField textFieldMail, TextField textFieldName){
        String urlPutGame="http://localhost:8080/Juegos/{CodJuego}";
        ...
        try {
            urlPutGame = urlPutGame.replace("{CodJuego", URLEncoder.encode(id,"UTF-8"));

            HttpPut putRequest = new HttpPut(urlPutGame) ;
            putRequest.setHeader("Accept", "application/json");
            putRequest.setHeader("Content-type", "application/json");

            String putJson= "{\"codJuego\":"+Integer.valueOf(id)+",\"nombre\":\""+name+"\",\"fechaLanzamiento\":\"2020-03-03\",\"precio\":20.20,\"pegi\":10,\"portada\":\"http://dumps.com/242x100.png/cc0000/ffffff\"}";

            StringEntity stringEntity = new StringEntity(putJson);
            putRequest.setEntity(stringEntity);

            String responseBody = httpClient.execute(putRequest, responseHandler);
            System.out.println("----------------------------------------");
